...
package com.youku.login.Zxing.camera;
...
//      }
  ...
  public void startPreview() {
    try {
		...
		YoukuUtil.showTips(R.string.camera_error_check_tip);
		e.printStackTrace();
	}
  }
  ...
  public Rect getFramingRect() {
      ...
      if (framingRect == null) {
        ...
        if (height < MIN_FRAME_WIDTH) {
          ...
          } else if (height > MAX_FRAME_WIDTH) {
            height = MAX_FRAME_WIDTH;
          }
        if (width>height) {
            width= height;
           }else {
        ...
        if (CaptureActivity.Wt*CaptureActivity.HT<=320*480) {
         	height=180;
         	width=180;
 		}
        int leftOffset = (screenResolution.x - width) / 2;
        int topOffset;
        if (screenResolution.y>1000) {
        	 topOffset = (screenResolution.y - height) / 4;
		}else {
        	 topOffset = (screenResolution.y - height) / 8;
...
//          Log.e(TAG, "screenResolution y:"+screenResolution.y);
          if (width < MIN_FRAME_WIDTH) {
            width = MIN_FRAME_WIDTH;
          } else if (width > MAX_FRAME_WIDTH) {
            width = MAX_FRAME_WIDTH;
...
//          Log.d(TAG, "Calculated framing height: " + height);
          int leftOffset = (screenResolution.x - width) / 2;
          int topOffset;
          if (screenResolution.y>1000) {
...
//          framingRect = new Rect(leftOffset, leftOffset/4, leftOffset + width, leftOffset + height);
    //  }else {
            framingRect = new Rect(leftOffset, topOffset, leftOffset + width, topOffset + height);
    //  }
//          Log.d(TAG, "Calculated framing rect: " + framingRect);
